...
Scene* create_scene() {
    ...
    scene->textureDirectory = NULL;
    return scene;
}

void set_scene_texture_directory(Scene* scene, const char* directory) {
    if (scene) {
        if (scene->textureDirectory) {
            free(scene->textureDirectory);
        }
        scene->textureDirectory = strdup(directory);
    }
}
...
SceneNode* create_node() {
    ...
    node->light = NULL;
    node->light_name = NULL;

    node->camera = NULL;
    node->camera_name = NULL;

    node->shader_program = NULL;
    node->vertex_shader = NULL;
    node->geometry_shader =  NULL;
    node->fragment_shader = NULL;

    node->should_draw_axes = true;
    glGenVertexArrays(1, &node->axesVAO);
    glGenBuffers(1, &node->axesVBO);
    node->axes_shader_program = NULL;
    node->axes_vertex_shader = NULL;
    node->axes_fragment_shader = NULL;
...
void set_shaders_for_node(SceneNode* node, Shader* vertex, Shader* geometry, Shader* fragment) {
    if (!node) {
        return; // If the node is NULL, we exit the function
    }

    // Set shaders for the current node
    node->vertex_shader = vertex;
    node->geometry_shader = geometry;
    node->fragment_shader = fragment;

    // Recursively set shaders for each child
    for (size_t i = 0; i < node->children_count; ++i) {
        set_shaders_for_node(node->children[i], vertex, geometry, fragment);
    }
}
...
void set_axes_shaders_for_node(SceneNode* node, Shader* vertex, Shader* fragment) {
    ...
    // Set shaders for the current node
    node->axes_vertex_shader = vertex;
    node->axes_fragment_shader = fragment;

    for (size_t i = 0; i < node->children_count; ++i) {
        set_axes_shaders_for_node(node->children[i], vertex, fragment);
    }
}
...
void draw_node_axes(SceneNode* node, const mat4 model, const mat4 view, const mat4 projection) {
    ...
    glUseProgram(node->axes_shader_program->programID);

    // Pass the model, view, and projection matrices to the shader
    GLint modelLoc = glGetUniformLocation(node->axes_shader_program->programID, "model");
    GLint viewLoc = glGetUniformLocation(node->axes_shader_program->programID, "view");
    GLint projLoc = glGetUniformLocation(node->axes_shader_program->programID, "projection");
...
void render_node(SceneNode* self, mat4 model, mat4 view, mat4 projection) {
    ...
    if (self->shader_program) {
        ...
        GLint modelLoc = glGetUniformLocation(self->shader_program->programID, "model");
        GLint viewLoc = glGetUniformLocation(self->shader_program->programID, "view");
        GLint projLoc = glGetUniformLocation(self->shader_program->programID, "projection");

        glUniformMatrix4fv(modelLoc, 1, GL_FALSE, (const GLfloat*)self->global_transform);
        glUniformMatrix4fv(viewLoc, 1, GL_FALSE, (const GLfloat*)view);
        glUniformMatrix4fv(projLoc, 1, GL_FALSE, (const GLfloat*)projection);
        ...
        if (self->meshes && self->mesh_count > 0) {
            for (size_t i = 0; i < self->mesh_count; ++i) {
                ...
                if (mesh && mesh->material) {
                    ...
                    GLint albedoLoc = glGetUniformLocation(self->shader_program->programID, "albedo");
                    GLint metallicLoc = glGetUniformLocation(self->shader_program->programID, "metallic");
                    GLint roughnessLoc = glGetUniformLocation(self->shader_program->programID, "roughness");
                    GLint aoLoc = glGetUniformLocation(self->shader_program->programID, "ao");

                    glUniform3fv(albedoLoc, 1, (const GLfloat*)&mat->albedo);
        ...
        if (self->light) {
            GLint lightPosLoc = glGetUniformLocation(self->shader_program->programID, "light.position");
            GLint lightColorLoc = glGetUniformLocation(self->shader_program->programID, "light.color");
            GLint lightIntensityLoc = glGetUniformLocation(self->shader_program->programID, "light.intensity");
            GLint lightColorSpecular = glGetUniformLocation(self->shader_program->programID, "light.specular");
            GLint lightColorAmbient = glGetUniformLocation(self->shader_program->programID, "light.ambient");
    ...
    if (self->should_draw_axes && self->axes_shader_program) {
        draw_node_axes(self, self->global_transform, view, projection);
    }

    // Render children
    for (size_t i = 0; i < self->children_count; i++) {
        render_node(self->children[i], self->global_transform, view, projection);
    }
}
