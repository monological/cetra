 ...
 *
 * `object_layer` is the caller's, because the layers are the caller's: which
 * layer means dynamic is decided by whoever configured the PhysicsSystem's
 * broadphase, and a number restated here would be a second copy of that
 * decision, silently wrong the day the first one is reordered.
 *
...
JoltRagdoll* jolt_ragdoll_create(JPC_PhysicsSystem* system, const RagdollBuild* parts, int count,
                                 uint32_t object_layer, uint32_t group_id);
