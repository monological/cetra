...
BoneCategory categorize_bone(const char* norm) {
    ...
    if (bone_contains(norm, "ctrl") || bone_contains(norm, "ik") ||
        bone_contains(norm, "pole") || bone_contains(norm, "target") ||
        bone_contains(norm, "helper")) {
        return BCAT_UNKNOWN;
    }
    ...
    if (bone_contains(norm, "finger") || bone_contains(norm, "thumb") ||
        ...
        bone_contains(norm, "ring") || (bone_contains(norm, "hand") &&
        (bone_contains(norm, "middle") || bone_contains(norm, "little")))) {
        return BCAT_FINGER;
    }

    // Toes
    if (bone_contains(norm, "toe")) return BCAT_TOE;

    // Foot/ankle
    if (bone_contains(norm, "foot") || bone_contains(norm, "ankle")) return BCAT_FOOT;
    ...
    if ((bone_contains(norm, "leg") && !bone_contains(norm, "upleg") &&
         !bone_contains(norm, "upperleg")) ||
        bone_contains(norm, "knee") || bone_contains(norm, "shin") ||
        bone_contains(norm, "calf")) {
        return BCAT_LOWER_LEG;
    }

    // Hand/wrist (but not if it's a finger bone)
    if (bone_contains(norm, "hand") || bone_contains(norm, "wrist")) return BCAT_HAND;
    ...
    if (bone_contains(norm, "neck")) return BCAT_NECK;
    ...
    // Spine/chest
    if (bone_contains(norm, "spine") || bone_contains(norm, "chest")) return BCAT_SPINE;

    // Hips/pelvis
    if (bone_contains(norm, "hip") || bone_contains(norm, "pelvis")) return BCAT_HIPS;

    // Root
    if (bone_contains(norm, "root") && !bone_contains(norm, "hip")) return BCAT_ROOT;
...
int get_bone_position(const char* norm, BoneCategory cat) {
    ...
    if (bone_contains(norm, "lower") || bone_contains(norm, "base")) return 1;
    if (cat != BCAT_FINGER) {
        if (bone_contains(norm, "middle") || bone_contains(norm, "mid")) return 2;
    }
    if (bone_contains(norm, "upper") || bone_contains(norm, "top")) return 3;

    // Spine special case: Mixamo uses Spine, Spine1, Spine2
    // where Spine=lower(1), Spine1=middle(2), Spine2=upper(3)
    if (cat == BCAT_SPINE) {
        if (bone_contains(norm, "spine2") || bone_contains(norm, "spine 2")) return 3;
        if (bone_contains(norm, "spine1") || bone_contains(norm, "spine 1")) return 2;
    ...
    if (cat == BCAT_FINGER) {
        ...
        if (len > 0) {
            ...
            if (last == '1' || last == 'a') return 1;
            if (last == '2' || last == 'b') return 2;
            if (last == '3' || last == 'c') return 3;
        }
    }

    // Numeric position (isolated numbers)
    if (bone_contains(norm, " 1") || bone_contains(norm, "1 ")) return 1;
    if (bone_contains(norm, " 2") || bone_contains(norm, "2 ")) return 2;
    if (bone_contains(norm, " 3") || bone_contains(norm, "3 ")) return 3;

    return 0;
}

int get_finger_id(const char* norm) {
    if (bone_contains(norm, "thumb")) return 1;
    if (bone_contains(norm, "index")) return 2;
    // "middle" for finger - check for handmiddle pattern (Mixamo style)
    if (bone_contains(norm, "handmiddle")) return 3;
    if (bone_contains(norm, "ring")) return 4;
    if (bone_contains(norm, "pinky") || bone_contains(norm, "little")) return 5;
...
int find_matching_bone_smart(Skeleton* skeleton, const char* anim_bone_name) {
    if (!skeleton || !anim_bone_name || skeleton->bone_count == 0) return -1;
    ...
    for (size_t i = 0; i < skeleton->bone_count; i++) {
        ...
        if (anim_pos != 0 && skel_pos != 0) {
            ...
            if (skel_pos == 1) score += 10.0f;
            else if (skel_pos == 2) score += 5.0f;
            // Position 3 (upper) gets no bonus
        }

        // Prefer "hips" over "pelvis" for hip bones
        if (anim_cat == BCAT_HIPS) {
            if (bone_contains(skel_norm, "hip")) score += 15.0f;
        }
