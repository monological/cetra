...
static size_t cwk_path_output_sized(char* buffer, size_t buffer_size, size_t position,
...
static size_t cwk_path_output_current(char* buffer, size_t buffer_size, size_t position) {
...
static size_t cwk_path_output_back(char* buffer, size_t buffer_size, size_t position) {
    // We output a "back" directory, which ahs two characters. This
    // character is currently not style dependant.
    return cwk_path_output_sized(buffer, buffer_size, position, "..", 2);
}

static size_t cwk_path_output_separator(char* buffer, size_t buffer_size, size_t position) {
    // We output a separator, which is a single character.
    return cwk_path_output_sized(buffer, buffer_size, position, separators[path_style], 1);
}

static size_t cwk_path_output_dot(char* buffer, size_t buffer_size, size_t position) {
    // We output a dot, which is a single character. This is used for extensions.
    return cwk_path_output_sized(buffer, buffer_size, position, ".", 1);
}

static size_t cwk_path_output(char* buffer, size_t buffer_size, size_t position, const char* str) {
    size_t length;
...
static void cwk_path_terminate_output(char* buffer, size_t buffer_size, size_t pos) {
    if (buffer_size > 0) {
        if (pos >= buffer_size) {
...
static bool cwk_path_is_string_equal(const char* first, const char* second, size_t first_size,
                                     size_t second_size) {
...
static bool cwk_path_get_first_segment_without_root(const char* path, const char* segments,
                                                    struct cwk_segment* segment) {
...
static bool cwk_path_get_last_segment_without_root(const char* path, struct cwk_segment* segment) {
...
static bool cwk_path_get_first_segment_joined(const char** paths, struct cwk_segment_joined* sj) {
    bool result;
    ...
    while (paths[sj->path_index] != NULL &&
           (result = cwk_path_get_first_segment(paths[sj->path_index], &sj->segment)) == false) {
        ++sj->path_index;
    }
...
static bool cwk_path_get_previous_segment_joined(struct cwk_segment_joined* sj) {
    ...
    do {
        ...
        if (sj->path_index == 0) {
            ...
            result =
                cwk_path_get_last_segment_without_root(sj->paths[sj->path_index], &sj->segment);
        }
...
static bool cwk_path_segment_normal_will_be_removed(struct cwk_segment_joined* sj) {
    enum cwk_segment_type type;
    int counter;
...
static bool cwk_path_segment_will_be_removed(const struct cwk_segment_joined* sj, bool absolute) {
    enum cwk_segment_type type;
...
static bool cwk_path_segment_joined_skip_invisible(struct cwk_segment_joined* sj, bool absolute) {
...
static size_t cwk_path_join_and_normalize_multiple(const char** paths, char* buffer,
                                                   size_t buffer_size) {
    size_t pos;
    ...
    do {
        ...
        // here.
        pos += cwk_path_output_sized(buffer, buffer_size, pos, sj.segment.begin, sj.segment.size);
    } while (cwk_path_get_next_segment_joined(&sj));
...
static void cwk_path_skip_segments_until_diverge(struct cwk_segment_joined* bsj,
                                                 struct cwk_segment_joined* osj, bool absolute,
    ...
    do {
        ...
        *other_available = cwk_path_segment_joined_skip_invisible(osj, absolute);
        ...
        // since they diverge.
        if (!cwk_path_is_string_equal(bsj->segment.begin, osj->segment.begin, bsj->segment.size,
                                      osj->segment.size)) {
            break;
        }
...
size_t cwk_path_get_relative(const char* base_directory, const char* path, char* buffer,
                             size_t buffer_size) {
    ...
    if (base_root_length != path_root_length ||
        !cwk_path_is_string_equal(base_directory, path, base_root_length, path_root_length)) {
        cwk_path_terminate_output(buffer, buffer_size, pos);
        return pos;
    }
    ...
    cwk_path_skip_segments_until_diverge(&bsj, &osj, absolute, &base_available, &other_available);
...
size_t cwk_path_change_root(const char* path, const char* new_root, char* buffer,
                            size_t buffer_size) {
    const char* tail;
...
size_t cwk_path_change_basename(const char* path, const char* new_basename, char* buffer,
                                size_t buffer_size) {
    struct cwk_segment segment;
    ...
    if (!cwk_path_get_last_segment(path, &segment)) {
        ...
        pos += cwk_path_output_sized(buffer, buffer_size, pos, new_basename, new_basename_size);
...
size_t cwk_path_change_extension(const char* path, const char* new_extension, char* buffer,
                                 size_t buffer_size) {
    struct cwk_segment segment;
    ...
    trail_size = cwk_path_output(buffer, buffer_size, pos + new_extension_size, segment.end);
...
size_t cwk_path_get_intersection(const char* path_base, const char* path_other) {
    ...
    if (!cwk_path_is_string_equal(path_base, path_other, base_root_length, other_root_length)) {
        return 0;
    }
    ...
    do {
        ...
        if (!cwk_path_is_string_equal(base.segment.begin, other.segment.begin, base.segment.size,
                                      other.segment.size)) {
...
size_t cwk_path_change_segment(struct cwk_segment* segment, const char* value, char* buffer,
                               size_t buffer_size) {
    ...
    cwk_path_output_sized(buffer, buffer_size, pos + value_size, segment->end, tail_size);
