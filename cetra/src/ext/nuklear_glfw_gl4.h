...
NK_API void nk_glfw3_device_create() {
    ...
    static const GLchar* vertex_shader =
        NK_SHADER_VERSION NK_SHADER_BINDLESS "uniform mat4 ProjMtx;\n"
                                             "in vec2 Position;\n"
                                             "in vec2 TexCoord;\n"
                                             "in vec4 Color;\n"
                                             "out vec2 Frag_UV;\n"
    ...
    /* Persistent mapped buffers */
    {
        ...
        dev->vert_buffer =
            (struct nk_glfw_vertex*)glMapNamedBufferRange(dev->vbo, 0, vb_size, flags);
...
NK_API void nk_glfw3_destroy_texture(unsigned int id) {
...
NK_API struct nk_context* nk_glfw3_init(GLFWwindow* win, enum nk_glfw_init_state init_state,
                                        int max_vertex_buffer, int max_element_buffer) {
    glfw.win = win;
...
NK_API void nk_glfw3_new_frame(void) {
    ...
    if (k_state[NK_KEY_TEXT_RESET_MODE] >= 0)
        nk_input_key(ctx, NK_KEY_TEXT_RESET_MODE, k_state[NK_KEY_TEXT_RESET_MODE]);
    ...
    if (k_state[NK_KEY_TEXT_INSERT_MODE] >= 0)
        nk_input_key(ctx, NK_KEY_TEXT_INSERT_MODE, k_state[NK_KEY_TEXT_INSERT_MODE]);
    if (k_state[NK_KEY_TEXT_REPLACE_MODE] >= 0)
        nk_input_key(ctx, NK_KEY_TEXT_REPLACE_MODE, k_state[NK_KEY_TEXT_REPLACE_MODE]);
