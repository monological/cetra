 ...
 * animation channels (scabbards, hair, straps). Each registered bone's tip
 * is simulated as a damped Verlet particle pulled toward its animated rest
 * position and constrained to the bone length; the resulting swing rotation
 * is applied on top of the animated pose between animation sampling and
 * skinning matrix computation.
 */

typedef struct SpringBoneParams {
    float stiffness;         // 0..1 pull fraction toward the animated pose (per 1/60s)
...
typedef struct SpringBoneJoint {
    ...
    int chain_index;       // index into system->chains (params lookup)
...
typedef struct SpringBoneChain {
    char* root_name; // owned copy, for debug output
    int first_joint; // range into system->joints
    int joint_count;
...
// Convenience wrappers that lazily create state->springs
