...
**Status: Tiers 1-4 are closed, and Tier 5 ran to completion (11.89-11.94).** Of Tier 4's rows only
...
| 55b | **T0.1 One denominator** | M | **BOOKED, measured, not built.** Every published number divides by its own count — GPU results the driver returned, frames the scope was entered, frames in the window — so a scope gated on something occasional prints a per-occurrence cost beside per-frame means and `TIMED` sums it in as though it ran throughout. `dir_shadow_fixture --gi-volume`: a one-frame 699 ms GI sweep in a 38-frame window gives **`TIMED` 17.9x its `FRAME`**, with nothing wrong in the timing code, under a header that asserts FRAME is the ceiling. Does not affect 55's forest numbers (every scope there runs every frame) and does not affect this tier's targets. The fix is one count for all three plus self-bracketing the frame clock, which deletes the `dt` parameter and makes `TIMED <= FRAME` an identity for the CPU column — and changes every number the profiler prints, so it wants its own verification pass. |
...
| 61 | **T6 The frame-stale scene graph** | M | Split out of 60, where it did not belong. Propagating the scene graph is ENGINE work that six apps hand-roll immediately before `render_current_scene` — after the shadow pass has already drawn from last frame's transforms. Needs a pre-render hook in both `engine_run` and the game framework rather than a fixed call, because `apps/render` recentres its model and `apps/shapes` animates its own matrix. Moves pixels in every app that casts a shadow, which is why it needs a spec and not a line. |
| 62 | **T7 Sampler declarations, and the end of Wall 1** | M | **The item 11.93 named first and did not ship**, and the largest thing Tier 5 left behind. All five of its gates are inside `main()`; not one gates a declaration, so `ltcTex`, `sheenTex`, `heightTex` and the Charlie sheen cube are still DECLARED on a `pbr-0` variant that provably cannot sample them — four of sixteen units. **Now reachable**: gating the area panel removed the last read of `ltcTex`, and `gen_shader_header.py` inlines every `#include` before the splice, so `#if` wraps a declaration exactly as easily as a statement. **This is the first mechanism in the tree that could DISSOLVE Wall 1 rather than dodge it.** CLAUDE.md documents six escapes and a roll call of features booked against that ledger; a lean variant with four units free is a seventh and a stronger one, because it returns units instead of routing around them. Two cautions: `heightTex` is aliased as `vtPageSurfaceTex` for layered materials (11.67), so the parallax gate must not touch that declaration; and the win is occupancy, which the GPU profiler prices badly — measure on wall clock. |
| 63 | **T8 Skinned variants** | S | `pbr_skinned` compiles the SAME fragment source as `pbr` — only the vertex stage differs — so every skinned material in every scene is on the uber-shader and gets none of 11.93's 17%. `apps/render`, the model viewer most goldens run through, is skinned throughout. Blocked on nothing but scope: with the mask already on `ShaderProgram`, admitting a second family member is a builder that carries its own vertex source. |
...
What is left is 32 (D0) and 37 (E7) -- the last L item with no
measurement demanding it, plus a foundation -- and 38 (E10), still S and still carrying `ssr_frag`'s
31,800 px. (This sentence read 29, 32, 37, 44 and "the re-scoped 36" until the sweep after 11.69: it
dropped 33, which was never built, and carried 44 and 36 after 11.62 and 11.69 shipped them.
29 went with 11.70, **and 33 with 11.73**.) **The one of those the roadmap argues against itself is
32 (D0)**, which
has had no scheduled consumer since 11.57, and 11.73 is the FIFTH feature in a row to need
