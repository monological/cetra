...
- **G-buffer**: **6** MRT attachments on one MSAA FBO, source-of-truth table
  ...
  - **RULE, and it is currently violated: nothing categorical or positional may live in a channel
    ...
    with the LEAF profile — reachable under `tree --msaa 4` rather than by default, since that app
    has run one sample since 11.88. **att2's `.z` is a view depth**, i.e. a position, and the average of two
    ...
    interactive path is *already* 1-sample + TAA (`apps/render/src/render.c`, the policy straddling
    `init_engine`) and structurally immune —
...
`.gltf`, generated by `assets/generators/gen_cornell_rooms.py`, matching all 13 other fixtures).
...
**Not built, because the effect already ships.** `assets/scenes/skin_shadow_fixture.cscn` was written to be
...
`engine_render_scene` invocation — seven times on a probe-capture frame. It went in its own block at
...
New: `cetra/src/ies.c/h`, `tools/gen_ies_table.py`, `assets/generators/gen_ies_fixture.py` (FIVE `.ies` files
...
| 64 | **G3 The interactive AA policy for masked content** | S | **DONE (11.102) having shipped no policy: the promotion is REFUSED on cost, and the row was wrong about the two things it leaned on.** The premise held — the interactive session runs TAA at **MSAA 1**, the one configuration where an alpha test cannot hold fractional coverage (`ladder`: 0.22 of the stored coverage at 358 flips, against 0.89 at 8 under MSAA 4 + A2C). What it cost to fix that: **forest's opaque row 26.6 → 55.3 ms (+108%, floor 1.74%)** and **`ivy_arcade` 17.3 → 105.6 ms, whole frame 31.0 → 122.4 (+296%, floor 1.82%)**, all pairs separated. **Two samples cost what four cost** (98.3 vs 105.6 on ivy), so the bill is the alpha-to-coverage MODE and there is no cheaper rung to promote onto; and **churn is non-monotonic** — 358 / 737 / 8 at one, two and four — so two is worse than one on look as well. **The row's first error: `apps/tree` does NOT run MSAA 4 + TAA.** It has run one sample unconditionally since 6ff9082b (11.88), chosen for cost AND because A2C on near-black leaves against a sunset traces a bright fringe round the canopy — so the app cited here as the counter-example is on the same path as the app with the problem, and never had the problem for a different reason than this row assumed. **The second: 11.31's "genuinely looks better" does not transfer.** It was measured at the accidental 2-sample target and under the fixed 0.02 threshold 11.87 called a bug; the 300% crops show **no raiden strands recovered** at four samples. What the MSAA path preserves so much better turns out to be coverage real content never has at a visible mip — 11.101 already found irregular textures never dither before their 1×1 tail — which is the sentence that should stop this row being re-opened on the 0.89 alone. **A byproduct worth more than the row: `--depth-prepass` REVERSES SIGN under A2C**, taking ivy's opaque row 105.6 → 47.9 ms where at one sample it moves 17.3 → 15.6 and does not separate. Row 31 and the CLI reference both record the prepass as losing everywhere in this corpus; both were measured at one sample and both stay true there. **Left standing deliberately:** 11.101's jitter keeps every consumer, since with no promotion the one-sample path is the only path. |

| 65 | **G4 The five apps that never chose an AA mode** | S | **DONE (11.103), and ONE of the five changed. The row was wrong about two of them and the exploration found an engine defect underneath a third.** `apps/gametest` took the policy and got the capture flags it never had — no headless, no frame limit, no screenshot, and no appearance in either suite, so before this nothing had ever rendered it. **`apps/spores` was REFUSED on measurement**: `particle_frag` declares one output, so a mote writes no motion vector and reprojects through whatever geometry is behind it — zero on a static camera. Its 32,000 motes stop being dots and become dashes. That is a question about the particle pass, not about a default, and the flags that measured it stay for whoever answers it. **`apps/splash` was never in scope and the row was wrong to include it**: it does not call `engine_run` at all, drawing SDF text straight to the default framebuffer, so its four samples come from the GLFW window hint and no app-side sample count reaches it. **`apps/shapes` keeps MSAA as a decision now written beside it** — multisampling is the natural AA for 2D line art, and nothing in that scene moves, so an accumulator would integrate only its own jitter while switching on the aux G-buffer, the resolve and eight temporal passes. **`apps/pcb` is not in the repository**, which is the row's third wrong claim and was found only when the comment written into it could not be committed: `apps/.gitignore` ignores the directory, so it is a local-only app that the CLAUDE.md example table still lists. The same reasoning applies to it and nothing ships. **The defect underneath: the TAA jitter was derived for perspective**, applied to the element that multiplies view-space z into clip x, which perspective divides back out and `glm_ortho` does not — so under the only camera mode those two apps use it shifted the raster by ~150 px a frame, scaled by depth. Unreachable by construction until `--ortho` gave the render app the camera mode, and now guarded by `taa-ortho`. **Amended by 11.104:** the flag shipped documented as DIAGNOSTIC ONLY, because the jitter was the one part of an orthographic frame this fixed and the rest was still perspective maths; row 66 is that list, closed. **The tempting fix was the wrong one and this row is the reason to say so**: resolving the default inside `init_engine` from the headless flag deletes every app's copy and fixes all five at once, and it would have switched TAA on for two apps that cannot use it and one the engine never reaches. **As booked, kept because two of its three claims were wrong and that is the record worth having:** *the engine's default is 4x MSAA with TAA off (`engine.c:168`), which is the HEADLESS configuration, and only render, forest and tree opt out — so spores, gametest, shapes, pcb and splash each pay four-sample rasterisation on the full six-attachment G-buffer with no temporal filter in every windowed session, precisely the state `apps/forest` names as the bug it fixed for itself. The case is proven for two and unexamined for three: spores and gametest are 3D scenes through the game framework where forest's reasoning transfers directly, while shapes, pcb and splash draw 2D primitives and SDF text where four samples may well be right. Do the two, look at the three, and only then consider the engine-side default.* The shape of that was right and the arithmetic was not: splash pays nothing on that G-buffer because it never binds it, and spores is the one of the two whose reasoning did not transfer. The startup half of the same finding was already fixed in 11.102. |
| 66 | **G5 Orthographic past the jitter** | S | **DONE (11.104), booked and closed in one motion because it was found rather than planned**: 11.103's review documented `--ortho` as diagnostic only, inside a flag's CLI entry and nowhere a reader of this ledger would see it. Exploration put the surface at SIX perspective-only formulas in the shaders — the depth inverse (one helper, twelve callers), the view-position reconstruction (duplicated in `froxel.glsl`), FIVE hand-rolled copies of the near-plane recovery, the world-to-screen length family, the view ray, and the per-fragment view vector at five sites, two of which the review missed (`gtao`, `ssr`) — plus four CPU sites: the cluster wedge, LOD selection, the occluder backface test, and the cascade fit, whose `tan(fov/2)` read a field `set_camera_orthographic` never touches. **The design is a branch on `projection[2][3]`** (0 under `glm_ortho`, −1 under `glm_perspective`; the jitter never lands there), with the perspective side of every branch the previous expression, so the goldens cannot move by construction; **a fallback was refused**, and there is no generalised formula. **The one exception was measured rather than argued**: a branch AROUND an unchanged GLSL divide is not byte-identical on this driver (water 130 px, skin 9 px, dof 12 px moved under if, ternary and coefficient forms alike), so `viewZFromNdcZ` keeps one coefficient form and two goldens were re-baked at last-bit drift. **Every bar was set against a measured reading on the broken build**: `ortho-view-eye` 107,665 px → 0 (V from the eye point), `ortho-ao` fraction 0 → ratio 0.99 (GTAO's screen radius at depth 150 sat under its early-out and read a flat 255), `ortho-dof` 31 → 6.7 codes, `ortho-cluster` ratio 0.09 → 0.72 (the wedge scaled by depth UNDER-assigned, swallowing every light into the innermost tiles — the plan predicted the opposite), `ortho-lod` the perspective ladder [13826, 5756, 1718] → equal, `ortho-lod-height` equal → 1718 < 13826. What `apps/shapes` had always rendered wrong — a view vector fanning 10.6° off forward at the frame edges — goes through the same helper now. **A post-hoc review found four more sites** three enumeration sweeps had missed, all in passes no ortho arm looked at: the froxel fog's history reprojection and phase direction, the aerial LUT's path, and water's optical path. Fixed through three more `depth.glsl` helpers with the goldens unmoved; the fog history got an arm (`ortho-fog-volume`, box/gap 0.96 → 0.60 against perspective's 0.59), the aerial LUT a hand check, and **water's eye-invariance arm refuted itself**: the plane crosses the near plane under a tilted parallel view, and the deferred projected grid spikes and moires once a branch is tried. Reverted; the grid stays deferred with the findings written down. **Deferred and recorded** in the CLI entry: skybox cube, sky background, ocean grid, cloud march, sort key, gizmo. |
| 67 | **G6 The engine as a submodule** | S | **DONE (11.105).** How a separate project builds the engine: `add_subdirectory` on a git submodule, or FetchContent. That already worked and was not documented, and three build defects hit the first consumer. `shader.h` included the generated shader header, so every file that reached `engine.h` compiled it and a `.glsl` edit rebuilt **66 objects**; only `program.c` includes it now, and the same edit rebuilds **1**. The output directories and the include tree were written to `CMAKE_BINARY_DIR`, which under a parent project is the parent's build root; they use `PROJECT_BINARY_DIR` now, the same path in tree. `CETRA_BUILD_APPS` defaulted on under a parent; it defaults to `PROJECT_IS_TOP_LEVEL` now. A fresh consumer build is **669 steps, 36 s cold**, the seven apps fewer than before, with no engine output in its `bin/` or `lib/` and the same frame; 30 goldens at 0 px. The `install()` rule was deleted: it had no export set and installed none of the libraries the archive links, so nothing could link against its prefix. A working install, and a Homebrew formula after it, is separate work for when someone needs a build they did not compile. Documented and not fixed: `BUILD_SHARED_LIBS` is forced off in the cache; adding cetra twice in one build fails on duplicate targets; `CETRA_BUILD_JOLTC=OFF` does not build, since `physics.c` includes JoltC unconditionally. A template project and a check script were in the first draft and were dropped; the three apps in `../apps` move to their own repositories on this path instead. |
| 68 | **G7 The public surface** | M | **DONE (11.106), the first of three API specs (11.107 "nothing fails silently" and 11.108 "settings and setters" follow).** The API grew with its own apps, and three passes over nine apps measured what that produced: 1,089 declared functions of which apps call 388, two naming conventions in equal use in `engine.h`, 91 headers all exported so `#include "cetra/engine.h"` pulled assimp and pthread into every consumer TU, and a create-then-init pair whose ordering rules lived in comments. **The boundary is enforced by the compiler, not by convention**: `cetra/CMakeLists.txt` allowlists 43 `src/*.h` plus `game/`, `procedural/`, `log.h` and `cwalk.h` as public; every other header forwards to `cetra/internal/<name>.h`, a second include root that `add_cetra_app` hands to in-tree apps and a consumer through `add_subdirectory` cannot name. `engine.h` no longer reaches assimp (the importer's `process_ai_*` went static), `DrawList` became a pointer on Scene and Engine so `scene.h` stops dragging the culler in, four typedefs are forward-declared, and the pass brackets no app calls moved to `engine_internal.h`. **One naming convention**: subject first, `engine_` / `scene_` / `node_` / `mesh_` / `material_` / `camera_` / `light_` / `game_`, 142 functions renamed by script in one commit across 62 files with no old name surviving outside `specs/`; `x_get_y` logs on a miss and `x_find_y` is silent, which closed the program-name-typo trapdoor. Return conventions: void for a mutator whose only failures are NULL or OOM, bool where a caller can act, NULL for factories and lookups, no `GLboolean` on the public surface. **The hybrid shape**, decided after a false start toward setters-for-everything: an object is CREATED from a description struct with designated initialisers and zero-means-default (`EngineConfig`, `CameraDesc`, `LightDesc`), a live object is MUTATED through a function only where the engine must react (`camera_set_position` / `_look_at` sync the orbit, `light_set_direction` / `_up` keep the authored-versus-world split, `light_set_intensity_units` converts), and everything else is a documented field. `create_engine` creates AND initialises, so `engine_init`, `engine_set_headless`, `engine_set_profiler` and the render app's sample-count-before / TAA-after straddle are gone, and `GameConfig` is the engine's config plus the two things the game layer owns. Seventeen plain-store builder setters deleted. **Verified as a no-op**: 30 goldens at 0 px at every phase, byte-identical captures from sprites, network, spores, tree and pcb (forest at its 2,500 px floor), the six gate groups the diff can reach, pcb built against every commit as the one external consumer, and the README's consumer example compiled and run through `add_subdirectory`. |
| 69 | **G8 Nothing fails silently** | M | **DONE (11.107), the second of the three API specs.** 11.106 measured a set of trapdoors a new app meets in its first hour, each of which rendered a plausible frame or died where nothing watched; this closes them. **The engine derives the camera** -- view and projection built once per frame right after the pre-render hook, before the shadow pass reads them -- so the matrix pair every app called at the end of its own hook is gone with its sixteen sites, the drag controller's and the config restore's deferred one, and the three apps that reached it only through the drag controller stop losing the aspect whenever the GUI held the pointer. `camera_set_position` / `camera_set_look_at` re-derive the orbit parameters, as 11.106's header promised; the four hand copies go and the derivation clamps its asin. **Upload at attach**: `node_add_mesh` uploads a mesh with vertices and nothing resident, tested through `Mesh.gpu_vertex_count` because the old `vao == 0` refusal was unreachable (`create_mesh` generates the VAO), so a mesh nobody uploaded drew from empty buffers; `node_upload_meshes` and its twelve sites go, `mesh_upload` measures the AABB (sixteen redundant app calls) and computes normals for a triangle mesh that has none, and the importer asks assimp for smooth normals and guards the one stream it read unguarded, so a model without normals loads lit instead of dereferencing NULL. **The draw list refuses with the reason, said when it changes** -- nothing uploaded, no material, a material another scene owns, no program, a failed uniform setup, or a draw mode the program's geometry stage cannot take, which was a `GL_INVALID_OPERATION` at the draw that nothing read; `Mesh.draw_refusal` holds the current reason for an app to read; `ShaderProgram.geometry_input` is asked of the linked program beside `instanced` and `sampler_count`, and is -1 for no stage since `GL_POINTS` is zero. **Materials register themselves**: the documented hook had no caller, so a material attached mid-run was never registered; the frame's draw-list build, hoisted to the frame top where a stamp compare makes the passes' own builds free, registers what it walks past before anything reads the registry, `Material.owner` replaces the linear scan and the first-frame sync alike, and the dirty marker is deleted. **`Engine.clear_color`** replaces the one literal in the attachment table (sprites drops the 80-unit black quad it built to stand in for it; its capture moves by exactly one 8-bit code over 1,222,050 px, from the quad's specular floor to the sketch's black); **a scene with no light, no environment and zero ambient warns once**, keyed on the light COUNT because a zero-intensity light is the documented way a fixture declines the rig; **`engine_run` and `game_run` draw the scene themselves when handed no render hook**, where before the whole frame ran and drew nothing; **`create_scene` makes a root** and `scene_set_root` frees the one it replaces, so eight apps and the importer stop building one. The XYZ gizmo's buffer moved from every node to the engine on the way (one 144-byte upload instead of one per node, which on forest was ~7,000 buffers nothing drew), so a node needs no GL to create and neither does a scene. **Verified as a no-op where it should be**: 30 goldens at 0 px at every phase, captures byte-identical from network, spores, tree and pcb (forest at its floor), the fourteen gate groups the diff can reach and the full suite once, pcb built against three phases and the review, and a throwaway consumer exercising each new behaviour: a triangle box on the shape program logs one line and no GL error, a box attached and never uploaded draws, a material created at frame 2 is registered by exit, a lightless scene warns once, every hook NULL draws the same bytes a callback did, a scene with no root of the app's own draws the same bytes, and a glTF with its NORMAL attribute deleted renders shaded. |
| 70 | **G9 Settings and setters** | S | **DONE (11.108), the third of the three API specs.** Every public struct's fields play one of three roles, and the header says which: ENGINE-OWNED (derived state, GL names, resources; read, never write), BY FUNCTION (the fields the engine must react to, each naming its function), SETTINGS (plain stores, written directly at any time). The small structs are laid out under the three banners; the feature-organised ones keep their order under one banner that states the rule by kind. **No plain-store setter survives**: the five Engine overlay and run-count setters, `engine_set_taa` (one store into a field the engine wrote directly in four places), `mesh_set_draw_mode`, six `transform_*` functions with no caller and the drag controller's two tuning setters go, every site a field write. Two that reacted became settings by moving the reaction to the read side: wireframe's cull switch joins its polygon mode at the frame top, and the XYZ gizmo stops propagating a flag and a program into every node -- the build takes the engine's switch and the submit the scene's program, so a node created after the overlay was turned on is drawn (pcb's X key drew the root's gizmo and none of its parts). **The fog ambient gets the one function it lacked**: its ownership flag moves from the sky to the PostFX beside the value, and `postfx_set_fog_ambient` stores and takes ownership in one call, which forest and spores wrote without and the sky stamped over. `node_set_position` for the sites writing three matrix cells; the billboard renderer's unlit path, the same shader with the floor at 1 and a white key, so sprites drops the light it carried only to cancel a tint; a canvas controller in `app.h`, the drag controller's 2D twin, replaces the thirty lines shapes and pcb each carried, over a `camera_translate` the four camera moves now share; three dead camera fields go with their snapshot rows, and the snapshot gains the fog ambient's ownership flag so a sky-driven session restores sky-driven. The reviews took the rule to the edges the spec had not walked: two uncalled scene array setters, two PostFX installs only the engine called, a text transform copy and four entity copies go; the two game installs free what they replace; a light's position gets the setter its direction had, since a light on no node is never re-derived. **Caught live**: nothing in the frame ever enabled back-face culling -- it came from the IBL bake's unconditional enable, a capture, a two-sided draw's restore or an app calling the wireframe setter with false -- so spores, with none of those, rendered its sphere unculled as a flat disc; the frame top enabling it moves that capture by 1,232,820 px to the picture the scene describes. Goldens 30 at 0 px at every phase, every other capture byte-identical or at its floor, pcb built and captured against every phase. |
...
What is left is **nothing again, and this time with the comparison sweep overdue rather than the
table merely empty.** 70 (G9) closed with 11.108, the last of the three API specs: every public
struct says which of its fields are the engine's, which react through a function, and which are
settings written directly; no plain-store setter survives, the two that reacted moved their
reaction to the frame top, the fog ambient got the function it lacked, and the 2D apps share a
canvas controller -- with one thing caught on the way: nothing in the frame had ever enabled
back-face culling, and spores had rendered its sphere as a disc. 69 (G8) closed with 11.107, the second of the three API specs: the camera
derived by the engine, upload at attach, a draw list that says why once, materials that register
themselves, a clear colour, a lightless warning, a loop that draws, and a scene with a root -- every
trapdoor 11.106 measured, closed with the goldens at 0 px. 68 (G7) closed with 11.106, the first of three API specs: a header allowlist
the compiler enforces, one naming convention, return conventions, and creation from description
structs in place of the create-then-init pair and seventeen builder setters -- a no-op by every
instrument, which is the point of it. 67 (G6) closed with 11.105: the engine could already be built as a
subdirectory of another project, nothing said so, and three build defects and a dead install rule
were in the way. 66 (G5) closed with 11.104, which fixed the perspective-only formulas behind the
jitter that 65 uncovered, one site at a time. 65 (G4) closed with 11.103, which changed exactly one of the five apps it
named: gametest took the policy and the capture flags it had never had, spores was refused on
measurement (its motes carry no motion vector and TAA turns them into dashes), splash turned out
never to touch the engine's framebuffer at all, and shapes and pcb keep MSAA as a decision now
written beside them. **The row's real yield was an engine defect none of it predicted** — the TAA
jitter was derived for perspective and destroyed the frame under the orthographic camera those two
apps use, unreachable until `--ortho` and now guarded by `taa-ortho`. 64 (G3) closed with 11.102
having shipped no policy at all: the promotion it existed to consider is refused on cost, and the
row's own two supports (that `apps/tree` runs four samples, that 11.31's groom "looks better" under
A2C) both turned out to be false. **That is three rows running whose EVIDENCE rotted faster than
their premise, and 65 makes four**, which is a fact about this table rather than about any row in
it: the premises keep surviving and the numbers under them keep going stale. 54 (G2) went with
11.100, its distribution half
built and its hashed half recorded as refused-for-now inside the row, and 11.101 -- the flicker
that half surfaced on the live TAA path -- booked 64 as the policy question it measured its way to
and did not decide. (This sentence read "nothing -- the table is closed" for the whole of 11.101,
with 64 sitting one screen above it unmentioned: the same rot this tail keeps recording. Before
that it read "54 (G2), and nothing else", and the prediction it carried held: the change landed
inside `texture_derive_levels` plus a recipe bump, with the cook absorbing the cost -- though the
...
4. New test content needed along the way: `--area-light` CLI flag (A2), ~~cornell-box GLB~~ **(A4:
   shipped as `assets/models/cornell_box.gltf` plus a `cornell_leak.gltf` variant, both from one generator
