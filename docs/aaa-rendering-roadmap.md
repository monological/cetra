...
| 67 | **G6 The engine as a submodule** | S | **DONE (11.105), booked and closed in one motion because the exploration findings were the work.** The question was how a project of its own consumes the engine, and the answer was already `add_subdirectory` on a submodule or FetchContent — nothing said so, and three build defects met the first consumer before its first line. `shader.h:6` pulled the 2.1 MB generated shader header into every TU that reached `engine.h`, so a `.glsl` edit rebuilt **66 objects** and a consumer's object depended on a file nothing ordered before it; now `program.c` alone includes it, **1 object**. `CMAKE_*_OUTPUT_DIRECTORY` and the include tree were written to `CMAKE_BINARY_DIR`, the consumer's build root under a parent; now `PROJECT_BINARY_DIR`, identical in tree. `CETRA_BUILD_APPS` defaulted ON under a parent too; now `PROJECT_IS_TOP_LEVEL`. A fresh consumer build fell from 689 steps to **578, 30 s cold**, its `bin/` and `lib/` empty of the engine, its frame unchanged; 30 goldens 0 px. **Deleted rather than finished**: an `install()` with no export set that installed none of the seven libraries the archive links, while five of their own install rules stayed live — a real install (and the Homebrew formula behind it) is its own spec, wanted when a consumer who will not build from source exists. **Documented, not fixed**: `BUILD_SHARED_LIBS` forced OFF in the cache; two consumers in one build collide on target names; `CETRA_BUILD_JOLTC=OFF` does not build (`physics.c` includes JoltC unconditionally). The first draft carried a committed template project and a check script and both were struck as bloat: the three apps in `../apps` become their own repositories on this path and are the proof. |
...
table merely empty.** 67 (G6) closed with 11.105, which found that the engine could already be
built as a subdirectory of another project and that nobody had said so, then removed the three
build defects a consumer met first and the install rule that pretended to be a fourth way in. 66
(G5) closed with 11.104, the perspective-only stack behind the jitter 65 uncovered, branched on
the projection matrix site by site. 65 (G4) closed with 11.103, which changed exactly one of the five apps it
