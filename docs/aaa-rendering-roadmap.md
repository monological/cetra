...
| 33 | D1 Clustered decals | L | Largest environment-art gap (narrowed slightly by 11.68 — see the entry). ~~Hard-blocked on D0.~~ **Blocked by a texture-layout choice, not by the ledger**: decals read in the opaque pass where unit 6 is idle, so a flat 2D atlas with computed tile UVs takes the alias and needs no freed unit. Only the `sampler2DArray` form is blocked. Price the flat atlas first. **And a NARROW slice of the ground half shipped elsewhere (11.68)**: a mark that can be MADE of a layer the surface already carries — a path, a track, a worn patch — is a splat-weight override with no texture at all, capped at 4 polylines of 16 points on one layered world-XZ material per scene. What remains here is most of the item: walls and props (no world-XZ splat to override), marks whose appearance is a picture rather than a material (posters, scorches), and any real density. |
