...
What is left is **54 (G2), and nothing else** -- 38 was rejected on investigation, 11.85 refused
32, and 37 went with 11.98. G2 is still M, and 11.99 sharpened its cheaper half in passing: alpha
distribution derives from exactly the inputs `texture-mips` already keys, so it is a change inside
`texture_derive_levels` plus a recipe bump, with the cook absorbing the cost after the first run.
When it closes, the column is empty again and the comparison habit's third run is due -- checking
the other documents first, per the Track F lesson below. (This sentence read 32, 37 and 38 until
