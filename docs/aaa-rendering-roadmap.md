...
| 65 | **G4 The five apps that never chose an AA mode** | S | Found by 11.102's review and booked rather than fixed, because the evidence covers two of the five. The engine's default is 4x MSAA with TAA off (`engine.c:168`, `postfx.c`), which is the HEADLESS configuration — and only render, forest and tree opt out of it, so `spores`, `gametest`, `shapes`, `pcb` and `splash` each pay four-sample rasterisation on the full six-attachment G-buffer with no temporal filter behind it in every windowed session. That is precisely the state `apps/forest` names as the bug it fixed for itself ("it shipped 4x MSAA with no temporal filter at all"). **The case is proven for two and unexamined for three.** Spores and gametest are 3D scenes through the game framework, where forest's and render's reasoning transfers directly and `GameConfig.msaa_samples` (11.102) is already the lever. Shapes, pcb and splash build the engine themselves and draw 2D primitives and SDF text, where four samples with no temporal filter may well be RIGHT — MSAA is the natural AA for line art — and where temporal jitter on static text has never been looked at by anyone. **So the tempting form of this is the wrong one**: resolving the default inside `init_engine` from the headless flag deletes all the duplication and fixes all five at once, and it writes a 3D-scene assumption into the engine that would render a perfectly plausible frame in the three cases where it is wrong. Do the two, look at the three, and only then consider the engine-side default. Note the startup half of the same finding is already fixed — render and forest allocated the whole G-buffer at 4x and rebuilt at 1x, which 11.102 corrected along with the trap that the policy's two halves straddle `init_engine` and cannot be joined. |
...
What is left is **65 (G4, the five apps that never chose an AA mode), and nothing else** -- an S
whose evidence covers two of its five apps, so it is booked rather than started. 64 (G3) closed
with 11.102 having shipped no policy at all: the promotion it existed to consider is refused on
cost, and the row's own two supports (that `apps/tree` runs four samples, that 11.31's groom
"looks better" under A2C) both turned out to be false, which is the third time in four specs that
a row's EVIDENCE rotted faster than its premise. 54 (G2) went with 11.100, its distribution half
