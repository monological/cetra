...
| 59 | **T4 Shader permutations** | L | The largest single lever on the opaque pass, which is **74% of the frame** once 11.90 and 11.91 have taken what they can (every CPU submission row together is ~11 ms). The fit says the pass is 21.6 ms fixed + 6.67 ms/Mpx. One program for every material, and DCE cannot help because the terminal `FragColor` write is unconditional — the shader says so itself. Calibrated by an in-tree measurement: two `vec3`s live across the body cost **11% of the opaque pass**. Carries `#version 410` + `textureGather` for the PCF/PCSS kernels, which the 4.1 context has always allowed. **And it inherits 11.91's depth-only instance block**: `model` alone is 64 B/instance against 192, so the same block holds 192 instances rather than 64 and the shadow pass's 10,951 fit in a third of the draws — blocked only because a second `CETRA_INSTANCE_MAX` means a second block declaration and a second program, which is this row. |
