...
| 64 | **G3 The interactive AA policy for masked content** | S | Booked by 11.101 and not built there, because it is a policy decision with a perf cost to measure rather than a lookup. The render app's interactive session enables TAA and drops to **MSAA 1** (the user's session that surfaced the flicker: `msaa_samples: 1`), which is the one configuration where an alpha-tested material cannot hold fractional coverage: a binary test under a clamped history keeps ~0.2 of 11.100's stored coverage and churns, where MSAA 4 + alpha-to-coverage keeps 0.89 at 8 flips a frame (the `ladder` group's numbers). `apps/tree` runs MSAA 4 + TAA and never had the problem. The question is whether a masked material in the scene should make the interactive default MSAA 4 + TAA, what that costs on forest, and whether the 1-sample path should keep 11.101's palliative or simply be documented as the wrong path for cutouts. Measure before deciding; 11.31 already recorded A2C-on-TAA as "genuinely looks better" on the raiden groom. |
