...
dialogue system to drive one are what remain of that row.
...
- **A game shipped on it** — packaging first, since a build nobody can install is
  the hard blocker (display modes closed in 12.15, root motion in 12.18 and the
  camera framework in 12.19); then the animation state machine, and whatever UI
  elements the game's own screens need.
