...
path:
  ...
  right  ALPHA_MASK cutoff 0.4, COLOR_0 alpha 0.6 -> above the cutoff
...
HALF = 1.0
GAP = 0.35  # clear space between the quads, so neither samples the other's edge
...
# White RGB so the vertex colour tints nothing -- sRGBToLinear(1) is 1 -- and
# alpha 0.6, comfortably above the 0.4 cutoff and comfortably below 1 so a
# fragment that still blends is unmistakable rather than marginal.
colors = [(1.0, 1.0, 1.0, 0.6)] * 4

pos_bytes = b"".join(struct.pack("<3f", *p) for p in positions)
nrm_bytes = b"".join(struct.pack("<3f", *n) for n in normals)
col_bytes = b"".join(struct.pack("<4f", *c) for c in colors)
idx_bytes = b"".join(struct.pack("<H", i) for i in indices)
buffer_bytes = pos_bytes + nrm_bytes + col_bytes + idx_bytes
...
gltf = {
    ...
    "scenes": [{"nodes": [0, 1]}],
    "nodes": [
        {"name": "opaque_ref", "mesh": 0, "translation": [-(HALF + GAP), 0.0, 0.0]},
        {"name": "masked", "mesh": 1, "translation": [HALF + GAP, 0.0, 0.0]},
    ],
    "meshes": [
    ...
    "bufferViews": [
        ...
        {"buffer": 0, "byteOffset": len(pos_bytes) + len(nrm_bytes), "byteLength": len(col_bytes),
         "target": 34962},
        {"buffer": 0, "byteOffset": len(pos_bytes) + len(nrm_bytes) + len(col_bytes),
...
cscn = {
    ...
    "camera": {"eye": [0.0, 1.0, 5.4], "target": [0.0, 1.0, 0.0], "fov": 45},
...
with open(scn, "w") as f:
    ...
    f.write("\n")
print("wrote", out, "and", scn, "(opaque reference + MASK at COLOR_0 alpha 0.6)")
