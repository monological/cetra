#!/usr/bin/env python3
"""Generate assets/sheen_fixture.gltf, the KHR_materials_sheen test asset.

A row of UV spheres exercising KHR_materials_sheen (a retroreflective Charlie
cloth lobe: velvet / satin), meant to be viewed under an HDR environment
(-e ...). Each cloth sphere is a DARK, matte diffuse base so the bright sheen
rim at grazing angles -- the whole point of the lobe -- stands out:
  - velvet_red  : near-black base (rough 0.9) + red sheen (0.8,0.2,0.2),
                  sheenRoughness 0.35 -> a broad soft red retroreflective rim
  - velvet_blue : same base + blue sheen (0.25,0.35,0.9), sheenRoughness 0.35
  - satin_gold  : same base + gold sheen (0.9,0.7,0.35), sheenRoughness 0.12
                  -> a tighter, sharper satin sheen
  - cloth_none  : the same base with NO KHR_materials_sheen, for an in-frame A/B
                  of the matte diffuse the sheen rim rides on (the global
...
def material(name, color, rough, sheen_color=None, sheen_rough=None):
    m = {
        "name": name,
    ...
    if sheen_color is not None:
        ks = {"sheenColorFactor": sheen_color}
        if sheen_rough is not None:
            ks["sheenRoughnessFactor"] = sheen_rough
        m["extensions"] = {"KHR_materials_sheen": ks}
    return m


base = (0.03, 0.03, 0.035)
materials = [
    material("velvet_red", base, 0.9, sheen_color=[0.8, 0.2, 0.2], sheen_rough=0.35),
    material("velvet_blue", base, 0.9, sheen_color=[0.25, 0.35, 0.9], sheen_rough=0.35),
    material("satin_gold", base, 0.9, sheen_color=[0.9, 0.7, 0.35], sheen_rough=0.12),
    material("cloth_none", base, 0.9),
]
...
gltf = {
    "asset": {"version": "2.0", "generator": "gen_sheen_fixture.py"},
