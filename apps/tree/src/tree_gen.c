...
// ---------------------------------------------------------------------------
// Mesh builder: growable parallel arrays that become a Mesh's arrays outright.
// ---------------------------------------------------------------------------
...
static bool mb_init(MeshBuilder* mb, size_t vres, size_t ires, bool want_colors) {
    ...
    mb->tan = malloc(mb->vcap * 3 * sizeof(float));
    mb->btn = malloc(mb->vcap * 3 * sizeof(float));
    mb->uv0 = malloc(mb->vcap * 2 * sizeof(float));
    mb->uv1 = malloc(mb->vcap * 2 * sizeof(float));
    mb->idx = malloc(mb->icap * sizeof(unsigned int));
    mb->ok = mb->pos && mb->nrm && mb->tan && mb->btn && mb->uv0 && mb->uv1 && mb->idx;
    if (want_colors) {
...
static void mb_free(MeshBuilder* mb) {
    free(mb->pos);
    free(mb->nrm);
    free(mb->tan);
    free(mb->btn);
...
static bool mb_grow_verts(MeshBuilder* mb) {
    ...
    float* p = safe_realloc(mb->pos, cap * 3 * sizeof(float));
    float* n = safe_realloc(mb->nrm, cap * 3 * sizeof(float));
    float* t = safe_realloc(mb->tan, cap * 3 * sizeof(float));
    float* b = safe_realloc(mb->btn, cap * 3 * sizeof(float));
    float* u0 = safe_realloc(mb->uv0, cap * 2 * sizeof(float));
    float* u1 = safe_realloc(mb->uv1, cap * 2 * sizeof(float));
    float* c = mb->want_colors ? safe_realloc(mb->col, cap * 4 * sizeof(float)) : NULL;
    if (p)
        mb->pos = p;
    if (n)
        mb->nrm = n;
    ...
    if (c)
        mb->col = c;
    if (!p || !n || !t || !b || !u0 || !u1 || (mb->want_colors && !c)) {
        mb->ok = false;
        return false;
    }
    mb->vcap = cap;
...
static unsigned int mb_vertex(MeshBuilder* mb, const vec3 p, const vec3 n, const vec3 t,
                              const vec3 b, float u0, float v0, float u1, float v1,
                              const float* rgba) {
    if (!mb->ok)
        return 0;
    ...
    memcpy(&mb->tan[i * 3], t, 3 * sizeof(float));
    memcpy(&mb->btn[i * 3], b, 3 * sizeof(float));
    mb->uv0[i * 2 + 0] = u0;
    mb->uv0[i * 2 + 1] = v0;
    mb->uv1[i * 2 + 0] = u1;
    mb->uv1[i * 2 + 1] = v1;
...
static void mb_tri(MeshBuilder* mb, unsigned int a, unsigned int b, unsigned int c) {
    if (!mb->ok)
        return;
...
// so nothing double-frees. free_mesh() frees them with free(), which is what
// malloc/safe_realloc produced.
static bool mb_transfer(MeshBuilder* mb, Mesh* mesh) {
