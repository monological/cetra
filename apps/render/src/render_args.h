...
typedef struct {
    ...
    // Both tri-state (-1 unset, 0 off, 1 on), because both engine defaults are
    // now ON: a plain "1 means enable" flag cannot express --no-oit once the
    // thing it would disable is already true.
    int oit;                    // Order-independent transparency for blend meshes
