...
RECIPES = [
    ...
    # --no-scene-file is INERT since 12.8 and kept deliberately. It was required
    # while the fixture had a sibling .cscn the app would auto-load, lighting the
    # scene with a second panel; the split moved the two apart, so there is
    # nothing beside the model to pick up. Left in place because removing it is a
    # behaviour bet against three baked goldens for no gain -- but it protects
    # nothing now, and a reader should not add a fourth recipe on its strength.
    # No -e <hdr>: the HDRs live outside the repo, so a golden needing one could
    # not be regenerated by anyone else.
