...
|---|------|--------|----------|
| 1 | A1 Clustered forward | L | The disruptive light-pipeline rewrite goes first so every later item edits the final loop + UBO layout once; deletes the biggest CPU cost immediately. |
| 2 | A2 LTC area lights | M | Signature environment feature; contained M on the now-stable loop; must precede DDGI so probes capture area-lit rooms. |
| 3 | A3 Contact shadows | S | Fully decoupled quick win; ships while DDGI groundwork starts. |
